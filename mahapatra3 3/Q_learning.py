...
if __name__ == "__main__":
    ...
    for i in range(EPISODES):
        ...
        while (not done):
            ...
            new_prediction = np.array([Q_table[(new_state,i)] for i in range(env.action_space.n)])
            new_action = np.argmax(new_prediction)
            max_q_value = new_prediction[new_action]
            Q_table[obs, action] = (1 - LEARNING_RATE) * (Q_table[obs, action]) + \
                                   (LEARNING_RATE * (reward + DISCOUNT_FACTOR * max_q_value))
